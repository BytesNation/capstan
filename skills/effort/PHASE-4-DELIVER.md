...
**Done when** the version decision is recorded, if the repository declares one, the note is reviewed, or its absence is reported as skipped, every finding is actioned or dismissed on the record, `<working copy>/.capstan/effort/` is deleted, and the operator has been told what shipped, what review found, and that the note and the default branch are theirs — the note to commit, the branch to push.
