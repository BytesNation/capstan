...
1. Dispatch the Courier. It packages what ships, drafts the recipient briefs, writes the knowledge-base note, and reports the note's absolute path. Capstan never runs git in the knowledge base; the note stays uncommitted.
...
5. Report to the operator: what shipped, what review found on the note, and what was actioned or dismissed. Say that the note is reviewed and ready, and that committing it is the operator's own action, to be done once, now — Capstan does not commit in the knowledge base, and this ordering is what keeps that commit from needing to be amended.
