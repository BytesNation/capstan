...
4. Delete `.capstan/effort/`. All of it: spec, plan, scratch, scout findings, review output, checkpoint drafts. This step moved here from the Courier for one reason: the Courier's own teardown was destroying the spec its review needs to grade against, and the Courier holds no Task tool, so it cannot dispatch its own Reviewer.

**Done when** the note is reviewed, or its absence is reported as skipped, every finding is actioned or dismissed on the record, and `.capstan/effort/` is deleted.
