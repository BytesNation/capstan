...
2. Dispatch Builders on the unblocked frontier. Every slice with no open blockers can go at once.
   ...
   - Move that slice's row in `tracker.md`, in the document home, to `building` as you dispatch it. A slice dropped at any point in this phase moves its row to `dropped` — never delete a row, dropped or merged.
...
**A slice that keeps coming back is asking a question nobody has asked yet.** Track one fix-dispatch counter per slice. It goes up every time a finding sends a Builder task back onto a slice already built once, whether the finding came from a slice review (step 5) or a verify return (step 8). It counts dispatches to a Builder, not review returns filed: a round you verify and merge yourself without ever writing a review still counts if it sent a fix back first, and a review round that changed nothing does not.

At the third fix dispatch on a slice, and at every one after it, ask whether that slice's design is wrong rather than its implementation. Ask it every time it fires, not once: a question that fires at three and goes quiet is how a slice reaches nine rounds with nobody having named why. Answer it unattended, the same as any other call this phase makes without the operator, and write one decision-log row every time it fires, whether or not the answer changed from the round before — a row that only appears on a changed answer looks identical to a round where nobody asked.

An answer to keep patching does not end the run. An answer that recuts or drops the slice changes the shape the operator locked at gate 2, so that answer ends the run here and reports to the operator instead. A recut slice's counter resets to zero: it is a different slice now, and carrying the old count in would trip the question on its very first round.

The record behind this rule is a slice that took nine rounds to fix, a procedure that took seven rounds before the right call was deleting it, a criterion that certified the same defect its own run had produced three rounds running, and a defect shape that shipped anyway after being named at three instances and hit again at five. Every one of those conclusions was reached only in hindsight, after the rounds were already spent. This is what asks the question while the slice is still open.
