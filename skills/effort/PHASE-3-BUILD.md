...
2. Dispatch Builders on the unblocked frontier. Every slice with no open blockers can go at once.
   ...
   - Move that slice's row in `tracker.md` to `building` as you dispatch it. A slice you cut instead of dispatching moves its row to `dropped` — never delete a row, cut or built.
...
**Done when** every slice in `plan.md` is built, reviewed, merged, and its worktree removed, the integration has been verified against the checks the repository declares, every review and verify return is filed, every review and verification finding is actioned or dismissed on the record, and every slice's row in `tracker.md` reads `merged` or `dropped` — none left at `planned` or `building`.
