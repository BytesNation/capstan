...
3. **Resolve each hunk.** Preserve both intents where possible. Where incompatible, pick the one matching the merge's stated goal and note the trade-off as a line in the decision log, in the document home, per `decision-record`. Do **not** invent new behaviour. Always resolve; never `--abort`.
