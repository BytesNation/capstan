...
**Gate 1, concept locked.** What we are building and why. What we are explicitly not building, which is usually the most useful section. The vocabulary we settled on, which is now in the glossary, `CONTEXT.md` in the document home (which is `<working copy>/.capstan/` unless configured otherwise), rather than only in this brief. Anything a Scout established that changed the shape.
...
Every checkpoint brief carries an **assumptions** section, drawn from the `assumed` lines in the decision log, `decisions.md` in the same document home. This is the mechanism that lets the crew keep moving through ambiguity instead of stopping. One line each, stated as an assumption rather than a fact, so a wrong one is cheap to catch here rather than expensive to discover later.
