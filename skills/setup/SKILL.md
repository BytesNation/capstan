...
**Back to `tracker.md`.** Reached whenever **The tracker-surface ask** settled on the default this run and the value currently in force, read at the top of that ask, named a board. Step 4 already confirmed the token can reach Projects v2, above, before this step reads anything, and left `capstan-tracker` untouched rather than removing it — this step decides whether there is a key to remove at all. Parse `<owner>`, `<repo>` and the project number out of that same value, `github:<owner>/<repo>#<project-number>` — the reverse runs on this ask's default branch, which never binds them the way the GitHub branch does.
...
Test resuming against completion, never existence, per 712: `tracker.md` already existing at the confirmed path is not enough on its own — an interrupted forward migration leaves that same file behind, with rows the board has since moved past. This run is resuming an interrupted teardown only when `tracker.md` is committed there and every row this read just found matches a row it already holds, same slice and same status — skip straight to teardown against whatever the board still carries; a row already torn down is simply absent from this read. Recover the commit its close comments name from `tracker.md`'s own commit rather than a fresh read of `HEAD`: `git -C <confirmed path> log -1 --format=%H -- tracker.md` names it, since nothing else touches that path between reconstruction and a future forward migration. Otherwise, validate every row this read found before anything is written, per 731: a status outside the four and a row with no milestone each stop the run here and report what was found. A `merged` row's issue is checked against the merge-commit comment format `TRACKER-GITHUB.md` declares — its stop condition and its non-conforming-comment handling are both declared there, per 738, rather than restated here. Nothing here is checked against git history — the board is what this reads, per 731.
...
On approval: write `tracker.md` whole at the confirmed path, one row per validated slice, under the column mapping `TRACKER-GITHUB.md` declares — `Commit` blank except on a `merged` row, where it carries the SHA its one conforming comment names. Verify it by reading the file back and confirming every row survived, then add and commit `tracker.md` by path alone, before touching the board — whatever steps 3 and 4 have already changed in this same run does not ride along with it, per 733. Carry that commit's SHA forward: it is the one every row's close names below.
