...
**Back to `tracker.md`.** Reached whenever **The tracker-surface ask** settled on the default this run and the value currently in force, read at the top of that ask, named a board. Step 4 already confirmed the token can reach Projects v2, above, before this step reads anything, and left `capstan-tracker` untouched rather than removing it — this step decides whether there is a key to remove at all.

Read the board back per [`TRACKER-GITHUB.md`](../effort/TRACKER-GITHUB.md)'s reading-back section, scoped to items whose `capstan Status` came back set. Everything else on the board is left exactly as it is, named in the report, per 730. If none carry it, there is nothing to migrate: remove the key and continue to step 6.

If `tracker.md` already exists at the home currently in force, this run is resuming an interrupted teardown — the reconstruction below already ran and was committed on its own, so skip straight to teardown, against whatever the board still carries; a row already torn down is simply absent from this read. Otherwise, validate every row this read found before anything is written, per 731: a status outside the four, a row with no milestone, or a `merged` row whose issue carries zero conforming comments or more than one, each stop the run here and report what was found. A non-conforming comment on a `merged` row does not stop it; report it alongside the commit that was read, per 738. Nothing here is checked against git history — the board is what this reads, per 731.

Describe the batch before writing anything — how many rows, how many milestones, that the board is torn down once `tracker.md` is written and verified — then read the Authority table in `skills/effort/SKILL.md` before writing or closing anything below: the board write and the issue close are the one row it governs here, since `tracker.md` coming back is a creation, never a delete. Declining says instead how many rows are on the board and that leaving `capstan-tracker` as it is strands them, per 700; the run stops there and the key is not removed.

On approval, or unattended where the table allows it: write `tracker.md` whole, one row per validated slice, under the column mapping `TRACKER-GITHUB.md` declares — `Commit` blank except on a `merged` row, where it carries the SHA its one conforming comment names. Verify it by reading the file back and confirming every row survived, then commit it alone, before touching the board — nothing else this run has changed yet needs to ride along, and this is what requirement 5 means by written and verified before any teardown begins, per 733. Carry that commit's SHA forward: it is the one every row's close names below.

Tear down one row at a time, issue closed before item removed so a crash between the two still leaves the row on the board to find on retry: comment on its issue naming the commit just committed, in the format `TRACKER-GITHUB.md` declares, close it with reason `completed` if it is not closed already, then remove its item from the board. Nothing here is deleted — not the issue, not the board, not the `Capstan Status` field, per 729.

Once every row under a milestone is torn down, close that milestone if it now carries no open issues at all, per 734:
...
One case commits twice. When step 5 tore rows down, `tracker.md` was already committed on its own, the moment it was written and verified, before teardown began. This step then carries only what teardown left for it — the `capstan-tracker` removal and the `.gitignore` line — as a second, later commit, not a rewrite of the first.
