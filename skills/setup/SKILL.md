...
Read the board back per [`TRACKER-GITHUB.md`](../effort/TRACKER-GITHUB.md)'s reading-back section, scoped to items whose `capstan Status` came back set. That call's own exit status decides whether the board was reachable, never its message — the failure this closes reported itself as `unknown owner type` under an exhausted rate limit, not as anything recognisable as one. A non-zero exit is TRACKER-GITHUB.md's **Unreachable stops the run**: stop here and report what the call returned, and it never reads as zero rows. Everything else on the board is left exactly as it is, named in the report, per 730. A clean exit is what lets the count of items carrying `capstan Status` mean anything: if that count is zero, there is nothing to migrate — remove the key and continue to step 6.
