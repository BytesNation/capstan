...
**Copied fields and derived fields.** Wherever a schema says to match neighbouring notes, here or in the file that declared it, that word covers two operations. Ask of each field whether its correct value is a property of the folder or a fact about this effort. A link to the folder's own map is a property of the folder, so copy it. A date, this note's type, and a tag naming the work are facts about this effort, so derive them: the tag comes from the project the effort belongs to, never from the organisation that owns the folder. Do not settle it by counting how many neighbours agree. A folder carries one value until a second project lands in it, and that is the moment the question first matters. Copying where the rule says derive is the failure mode, because a neighbour's value is always plausible and nothing surfaces the mistake until one search returns two projects at once.
