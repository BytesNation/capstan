...
5. **Phase four delivers.** The Courier packages the output and writes the permanent note. Committing it is the operator's, once the note review passes. The Courier never sends anything to anyone. That part stays yours.
...
**The knowledge-base note is always reviewed whole.** Capstan never commits the note, so it never has a fixed point to diff against. The Reviewer reads the full note and says so in the report.
